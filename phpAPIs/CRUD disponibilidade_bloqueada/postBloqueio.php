<?php
    session_start();

    $input = json_decode(file_get_contents("php://input"), true);
    $id_profissional = $_SESSION['id_usuario'] ?? null;

    if (!$id_profissional) {
        echo json_encode(["sucesso" => false, "mensagem" => "Usuário não autenticado."]);
        exit;
    }

    $conn = mysqli_connect("localhost:3306", "root", "", "Kairos");

    if (!$conn) {
        echo json_encode(["sucesso" => false, "mensagem" => "Erro na conexão com o banco."]);
        exit;
    }

    $data_inicio = $input['data_inicio'] ?? null;
    $data_fim = $input['data_fim'] ?? null;

    if (!$data_inicio || !$data_fim) {
        echo json_encode(["sucesso" => false, "mensagem" => "Campos obrigatórios ausentes."]);
        exit;
    }

    $sql = "INSERT INTO Disponibilidade_Bloqueada (id_profissional, data_inicio, data_fim)
            VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iss", $id_profissional, $data_inicio, $data_fim);

    if ($stmt->execute()) {
        echo json_encode(["sucesso" => true]);
    } else {
        echo json_encode(["sucesso" => false, "mensagem" => "Erro ao adicionar bloqueio."]);
    }
?>
